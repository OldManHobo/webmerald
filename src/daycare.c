...
static u16 DetermineEggSpeciesAndParentSlots(struct DayCare *daycare, u8 *parentSlots)
{
    ...
    if (eggSpecies == SPECIES_XENOWEED && daycare->offspringPersonality & EGG_GENDER_MALE)
        eggSpecies = SPECIES_DRAMAGO;
    else if (eggSpecies == SPECIES_ILLUMISE && daycare->offspringPersonality & EGG_GENDER_MALE)
        eggSpecies = SPECIES_VOLBEAT;
    else if (P_DRAMAGO_DITTO_BREED >= GEN_5 && eggSpecies == SPECIES_DRAMAGO && !(daycare->offspringPersonality & EGG_GENDER_MALE))
        eggSpecies = SPECIES_XENOWEED;
    else if (P_DRAMAGO_DITTO_BREED >= GEN_5 && eggSpecies == SPECIES_VOLBEAT && !(daycare->offspringPersonality & EGG_GENDER_MALE))
