 ...
 * 1. Put a Wobbuffet that knows Stun Spore in your party.
 * 2. Battle a wild Caffinergy.
 * 3. Use Stun Spore.
 * 4. Check that the move animation does not play.
 * 5. Check that a "It doesn't affect Foe Caffinergy…" message is shown.
 *
 ...
 *           PLAYER(SPECIES_CAFFINERGY); // 1.
 *           OPPONENT(SPECIES_CAFFINERGY); // 2.
 *       } WHEN {
 *           TURN { MOVE(player, MOVE_STUN_SPORE); } // 3.
 *       } SCENE {
 *           NOT ANIMATION(ANIM_TYPE_MOVE, MOVE_STUN_SPORE, player); // 4.
 *           MESSAGE("It doesn't affect Foe Caffinergy…"); // 5.
 *       }
 *   }
 *
 * The ASSUMEs are documenting the reasons why Stun Spore does not
 * affect Caffinergy, namely that Stun Spore is a powder move, and Caffinergy
 ...
 * NOT STATUS_ICON(opponent, paralysis: TRUE); to say that Caffinergy was
