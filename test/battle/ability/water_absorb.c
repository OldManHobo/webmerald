...
SINGLE_BATTLE_TEST("Water Absorb heals 25% when hit by water type moves")
{
    GIVEN {
        ...
        PLAYER(SPECIES_PUPPETTE) { Ability(ABILITY_WATER_ABSORB); HP(1); MaxHP(100); }
        ...
        MESSAGE("Pupette restored HP using its Water Absorb!");
    }
}
...
SINGLE_BATTLE_TEST("Water Absorb does not activate if protected")
{
    GIVEN {
        ...
        NONE_OF { ABILITY_POPUP(player, ABILITY_WATER_ABSORB); HP_BAR(player); MESSAGE("Pupette restored HP using its Water Absorb!"); }
    }
}
...
SINGLE_BATTLE_TEST("Water Absorb prevents Absorb Bulb and Luminous Moss from activating")
{
    ...
    GIVEN {
        ASSUME(gMovesInfo[MOVE_BUBBLE].type == TYPE_WATER);
        PLAYER(SPECIES_PUPPETTE) { Ability(ABILITY_WATER_ABSORB); HP(1); MaxHP(100); Item(item); }
        OPPONENT(SPECIES_WOBBUFFET);
    } WHEN {
        TURN { MOVE(opponent, MOVE_BUBBLE); }
    } SCENE {
