...
{
    GIVEN {
        PLAYER(SPECIES_BANSHARK) { MaxHP(80); HP(80); Ability(ABILITY_GLUTTONY); Item(ITEM_MICLE_BERRY); }
        OPPONENT(SPECIES_WOBBUFFET);
    } WHEN {
        ...
        ANIMATION(ANIM_TYPE_GENERAL, B_ANIM_HELD_ITEM_EFFECT, player);
        MESSAGE("Banshark boosted the accuracy of its next move using Micle Berry!");
    }
}
